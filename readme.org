...
This is a fork of the base miryoku for Qile's personal usage. It involves additions to support rotary encoders via editing `miryoku_layer_list.h` and `miryoku.dtsi` as well as his preferences for `custom_config.h`.
...
[[#additional-and-experimental-features][Additional and Experimental Features]] include [[#caps-word][caps word]], [[#customisation][customisation]], [[#key-emulation-combos][key emulation combos]] and [[#mouse-keys][mouse keys]].
